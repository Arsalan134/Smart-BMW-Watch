...
const st77916_lcd_init_cmd_t vendor_specific_init_default[] = {
...
const st77916_lcd_init_cmd_t vendor_specific_init_version_2[] = {
    {0xF0, (uint8_t[]){0x28}, 1, 0},
    {0xF2, (uint8_t[]){0x28}, 1, 0},
...
const size_t vendor_specific_init_default_size = sizeof(vendor_specific_init_default) / sizeof(st77916_lcd_init_cmd_t);
const size_t vendor_specific_init_version_2_size = sizeof(vendor_specific_init_version_2) / sizeof(st77916_lcd_init_cmd_t);
...
esp_err_t esp_lcd_new_panel_st77916(const esp_lcd_panel_io_handle_t io,
  ...
  st77916 = (st77916_panel_t*)calloc(1, sizeof(st77916_panel_t));
...
static esp_err_t panel_st77916_del(esp_lcd_panel_t* panel) {
  ...
  if (st77916->reset_gpio_num >= 0) {
    gpio_reset_pin((gpio_num_t)st77916->reset_gpio_num);
  }
  free(st77916);
  return ESP_OK;
}
...
static esp_err_t panel_st77916_reset(esp_lcd_panel_t* panel) {
  ...
  if (st77916->reset_gpio_num >= 0) {
    gpio_set_level((gpio_num_t)st77916->reset_gpio_num, st77916->flags.reset_level);
...
static esp_err_t panel_st77916_mirror(esp_lcd_panel_t* panel, bool mirror_x, bool mirror_y) {
  ...
  ESP_RETURN_ON_ERROR(tx_param(st77916, io, LCD_CMD_MADCTL, (uint8_t[]){st77916->madctl_val}, 1), TAG, "send command failed");
  return ESP_OK;
}
...
static esp_err_t panel_st77916_swap_xy(esp_lcd_panel_t* panel, bool swap_axes) {
  ...
  esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){st77916->madctl_val}, 1);
  return ESP_OK;
}
